...
export default function MiCuenta({ userType = 'mexico', migrantName: migrantNameProp, userName: userNameProp, userLastName: userLastNameProp }) {
  const migrantName = migrantNameProp || 'Tu familiar';
  const userName = userNameProp || '';
  const userLastName = userLastNameProp || '';
  ...
  // Pre-populate titular (first member) with user data from Supabase
  useEffect(() => {
    if (!initialized && (userName || userLastName)) {
      setMembers(prev => {
        const updated = [...prev];
        updated[0] = { ...updated[0], nombre: userName || '', apellido: userLastName || '' };
        return updated;
      });
      setInitialized(true);
    }
    ...
    <div style={S.page}>
      ...
      <div style={S.px}>
        ...
        <div style={{ background: 'linear-gradient(135deg, rgba(6,182,212,0.08), rgba(236,72,153,0.06))', border: '1px solid rgba(6,182,212,0.15)', borderRadius: 16, padding: 20, marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}><span style={{ fontSize: 13, color: 'rgba(255,255,255,0.45)' }}>Pagada por</span><span style={{ fontSize: 13, fontWeight: 700, color: '#06B6D4' }}>{migrantName} (EE.UU.)</span></div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'rgba(236,72,153,0.06)', border: '1px solid rgba(236,72,153,0.12)', borderRadius: 12, padding: 14, marginBottom: 8 }}>
          <I.Heart /><p style={{ fontSize: 12, color: 'rgba(255,255,255,0.55)', lineHeight: 1.5 }}><span style={{ color: '#06B6D4', fontWeight: 700 }}>{migrantName}</span> paga esta suscripción para que tú y tu familia estén protegidos.</p>
        </div>
        ...
        <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)', textAlign: 'center', marginBottom: 20 }}>La administración de pagos es gestionada por {migrantName} desde EE.UU.</p>
      </div>
