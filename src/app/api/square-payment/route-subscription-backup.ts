...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // ════════════════════════════════════════════════════════════════════════
    // 6️⃣ SQUARE SUBSCRIPTION AUTO-CHARGES - NO MANUAL PAYMENT NEEDED
    // ════════════════════════════════════════════════════════════════════════
    console.log('ℹ️ [SQUARE] Subscription created - Square will auto-charge on billing cycle');
    console.log('ℹ️ [SQUARE] No manual payment needed - subscription handles billing');
    
    // Square automáticamente cobra cuando crea la suscripción
    // No necesitamos hacer un pago manual adicional
    const paymentId = `auto_${subscriptionId}`;
    ...
    // Save payment record (subscription auto-payment)
    console.log('💾 [SUPABASE] Guardando payment record (subscription auto-payment)...');
    const { data: paymentRecord, error: paymentError } = await supabase
      ...
      .insert({
        ...
        amount_cents: 1200,
        currency: 'USD',
        status: 'PENDING', // Square cobrará automáticamente
      })
      .select()
      .single();

    if (paymentError) {
      console.error('❌ [SUPABASE] Payment record save failed:', JSON.stringify(paymentError, null, 2));
      console.error('❌ [SUPABASE] Error code:', paymentError.code);
      console.error('❌ [SUPABASE] Error message:', paymentError.message);
    } else {
      console.log('✅ [SUPABASE] Payment record saved:', paymentRecord?.id);
    }
