 ...
 * 
 * Guarda lead de Landing (Page 1) antes de registro completo
 * Permite remarketing a usuarios que abandonan
 * Trackea source de tráfico para analytics
 * 
 * @method POST
 * @route /api/pre-checkout
 */

import { NextRequest, NextResponse } from 'next/server';
import { getSupabaseClient } from '@/lib/supabase';
import { mapLandingToPreCheckout } from '@/lib/field-mapper';
import type { LandingFormData } from '@/lib/field-mapper';

export async function POST(req: NextRequest) {
  try {
    // 1. Parse body
    const body: LandingFormData = await req.json();
