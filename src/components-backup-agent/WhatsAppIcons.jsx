...
export const WhatsAppIconGreen = ({ className = "w-12 h-12" }) => {
  return (
    <svg 
      className={className} 
      viewBox="0 0 200 200" 
    ...
    >
      {/* Fondo circular verde WhatsApp con gradiente */}
      <defs>
        <linearGradient id="whatsappGradient" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" stopColor="#25D366" />
          <stop offset="50%" stopColor="#20C65A" />
          <stop offset="100%" stopColor="#1BAF4F" />
        </linearGradient>
        <filter id="shadowWhatsApp" x="-50%" y="-50%" width="200%" height="200%">
      ...
      <circle cx="100" cy="100" r="95" fill="url(#whatsappGradient)" filter="url(#shadowWhatsApp)"/>
...
// Botón de WhatsApp con VERDE OFICIAL
export const WhatsAppButtonGreen = ({ onClick, href, children = "Chatea con Nosotros" }) => {
  ...
  return (
    <Component
      {...props}
      className="inline-flex items-center gap-3 px-6 py-3 bg-gradient-to-r from-[#25D366] to-[#20C65A] hover:from-[#20C65A] hover:to-[#1BAF4F] text-white rounded-xl font-bold shadow-lg hover:shadow-xl transition-all transform hover:scale-105"
    >
      <WhatsAppIconGreen className="w-8 h-8" />
      <span>{children}</span>
    </Component>
  );
};
...
export const WhatsAppFloatingButton = ({ href, variant = "green", className = "" }) => {
  const Icon = variant === "magenta" ? WhatsAppIconMagenta : variant === "cyan" ? WhatsAppIconCyan : WhatsAppIconGreen;
  const bgColor = variant === "magenta" 
    ...
    : "bg-gradient-to-br from-[#25D366] to-[#20C65A] hover:from-[#20C65A] hover:to-[#1BAF4F]";
  ...
  return (
    <a
      ...
      className={`fixed bottom-6 right-6 ${bgColor} p-4 rounded-full shadow-2xl hover:shadow-green-500/50 transition-all transform hover:scale-110 z-50 group ${className}`}
